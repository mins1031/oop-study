...
## 상속보단 조립
  ...
  - 조립
    ...
    - 조립의 의미가 애매했는데 일단 어렵게 생각할 거 없이 해당 기능을 가지는 클래스를 리생성해 만들고 상속이 필요했던 클래스의 필드로 새로만든 기능 클래스를 들고 있게 한다는 것 이라고 이해했다.

## 기능과 책임 분리
  - 기능은 곧 책임이다. 분리한 각 기능을 알맞는 위치에 분배하는 것이 객체지향 설계의 기본이다.
  - 책임에 따라 알맞게 코드 분리가 필요하다.
  - 책임 분배/분리 방법은 4가지 정도가 있다.
    1) 패턴 적용
      - 전형적인 역할 분리 패턴을 적용하는것
        - 간단한 웹 : 컨트롤러, 서비스, DAO
      - 복잡한 도메인 : 엔티티, 벨류, 레포지토리, 도메인 서비스
      - AOP : Aspect
      - Gof : 팩토리, 빌더, 전략,탬플릿 메서드, 프록시, 데코리터등
    2) 계산분리
      - 만약 계산하는 로직이 서비스로직에 있다면 계산하는 클래스를 따로 만들어 적용한다
    3) 연동분리
      - 네트워크, 메시징, 파일등 연동처리 코드를 분리한다.
    4) 조건분기는 추상화한다
      - 형태가 유사한 연속적 if-else는 추상화를 고민해 볼 것.

    - 역할 분리가 잘 되면 테스트가 용이해짐 -> 만약 계산분리 내용처럼 계산 분리가 안된 로직은 계산에 대한 테스트를 할때 여러 부가적인 요소들도 봐야하지만 계산 처리로직이 분리된 상황에선 계산 로직이 있는 메서드만 테스트하면 되기 때문에 테스트가 용이하다

## 의존과 DI
  ### 의존
    - 기능 구현을 위해 다른 구성 요소를 사용하는 것 -> 객체 생성, 메서드 호출, 데이터 사용 등등
    - 의존은 변경이 전파될 가능성을 의미한다.
      - 의존하는 대상이 바뀌면 바뀔 가능성이 높아진다. -> 호출하는 메서드의 파라미터가 변경, 호출하는 메서드의 리턴 타입이 변경 등등
    - 의존 대상이 많다면??
      1) 기능이 많은 경우 : 한 클래스에서 많은 기능을 제공하는 경우가 많다.
        - 각 기능(메서드)마다 의존하는 대상이 다를수 있다.
        - 기능 끼리 의존하고 있다면 한 기능 변경이 다른 기능에 영향을 줄 수 있고 테스트 시에도 불편함을 겪을 가능성이 크다
        - 이러한 경우 기능별로 아예 클래스 자체를 분리하는 방법을 고려해 볼수 있겠다. -> 회사에서 service를 조회용과 저장,수정,삭제용 으로 나눠서 구현한 것과 동일하다고 볼 수 있다.
    - 의존 대상 객체를 직접 생성하면?
      - 생성 클래스가 바뀌면 의존하는 코드도 바뀐다.
      - 의존대상 객체를 직접 생성하지 않는 방법은 대표적으로 의존주입 = DI가 있다
  ### DI = 의존주입
    - 외부에서 생성자나 메서드를 통해 의존 객체를 주입한다.
  ### DI의 장점
