...
- 다만 훌륭한 퍼블릭 인터페이스를 얻기 위해선 책임주도 설계방법을 따르는것 만으로 부족하고 유연하고 재사용 가능한 퍼블릭 인터페이스를 만드는데 도움이되는 설계 원칙과 기법을 익히고 적용해야 한다
  ...
  - 지금은 퍼블릭 인터페이스의 품질에 영향을 미치는 원칙과 기법에 관해 살펴본다
   ...
   - **원칙을 아는것보다 더 중요한것은 원칙이 언제 유용하고 언제 유용하지 않은지를 판단할수 있는 능력을 기르는 것이다.**
...
- **객체지향 설계의 핵심은 협력을 위해 필요한 의존성을 유지하면서도 변경을 방해하는 의존성은 제거하는데 있다**
 ## 8-1 의존성에 대한 이해
   ...
   ### 컨택스트 독립성
     ...
     - 클래스가 특정 문맥에 강하게 결합될 수록 다른 문맥에서 사용하기 어려워 진다. 클래스가 사용될 특정한 문맥에 대해 '최소한의 가정'만으로 이뤄져 있다면 다른 문맥에서 재사용하기가 수월해지고 이를 '컨택스트 독립성' 이라고 한다
 ...
 ## 8-2 유연한 설계
  ...
  ### 지식이 결합을 낳는다
    ...
    - 한 요소가 다른 요소에 대해 더 많은 정보를 알고 있을 수록 두 요소는 강하게 결합되고 반대로 한 요소가 다른 요소에 대해 더 적은 정보를 알고 있을 수록 두 요소는 약하게 결합된다.
  ...
  ### 추상화에 의존해라
    ...
    - 일반적으로 추상화와 결합도의 관점에서 의존 대상을 다음과 같이 구분하는 것이 유용하다(아래 일수록 클라이언트가 알아야 하는 지식의 양이 적어지기 때문에 결합도가 느슨해진다)
      ...
      - 추상 클래스 의존성
      - 인터페이스 의존성
    - 다시 말해 의존하는 대상이 더 추상적일 수록 결합도는 더 낮아진다는 점이 핵심이다.
  ### 명시적인 의존성
    - 아래 코드는 한 가지 실수로 인해 결합도가 불필요 하게 높아졌다.
    ```
    public class Movie {
      ...
    ...
    ```
    - 위 코드는 Movie가 DiscountPolicy 뿐만 아니라 AmountDiscountPolicy 에도 의존하고 있다.
    - 이 예제에서 알 수 있듯이 겹할도를 느슨하게 하기 위해 인스턴스 변수의 타입을 추상클래스나 인터페이스로 선언하는 것만 하는 것은 당연히 부족하고 '클래스 내에서 구체 클래스에 대한 모든 의존성을 제거 해야한다'
    - 위의 내용을 해결하기 위해선 의존성을 명시적으로 드러내야 한다. 의존성을 드러내면 코드를 직접 수정해야 하는 위험을 피할 수 있다.
    ```
    ...
    ```
    - 위처럼 의존하고 있는 내용은 왠만해선 명시를 해주는 것이 좋다고 한다(이 부분은 애매..)
# 9장 유연한 설계
> 앞장에서 유연하고 재사용 가능한 설계를 만들기 위해 적용할 수 있는 다양한 의존성 관리 기법들을 소개했다. 이번 장은 이 기법들을 원칙이라는 관점에서 정리한다.
 ## 9-1 개방-폐쇄 원칙
 > 로버트 마틴은 확장 가능하고 변화에 유연하게 대응할 수 있는 설계를 만들 수 있는 원칙중 하나로 개방-폐쇄 원칙을 고안했다
 '소프트웨어 개체(클래스, 모듈, 함수등등)는 확장에 대해 열려있어야 하고 수정에 대해서는 닫혀 있어야 한다.'
 - 여기서 키워드는 '확장' 과 '수정'이다. 이둘은 순서대로 어플리케이션의 '동작' 과 '코드'의 관점을 반영한다.
   - 확장에 대해 열려있다 : 어플리케이션의 요구사항이 변경될 때 이 변경에 맞게 새로운 동작을 추가해서 어플리케이션의 기능을 확장할 수 있다.
   - 수정에 대해 닫혀 있다 : 기존 코드를 수정하지 않고도 어플리케이션의 동작을 추가하거나 변경할 수 있다
  ### 컴파일타임 의존성을 고정시키고 런타임 의존성을 변경하라
  > 사실 개방-폐쇄 원칙은 런타임 의존성과 컴파일타임 의존성에 관한 이야기다
  - 현재 예제의 설계는 새로운 할인 정책을 추가해서 기능을 확장할 수 있도록 허용한다. 따라서 확장에 대해 서는 열려있다.
  - 현재의 설계는 기존 코드를 수정할 필요 없이 새로운 클래스를 추가하는 것만으로 새로운 할인 정책을 확장할 수 있다. 따라서 수정에 대해서는 닫혀있다.
...
# 10장 상속과 코드 재사용
> 해당 장은 클래스의 상속과 합성에 대해 이야기 한디. 객체지향 패러다임에서 코드를 재사용하기 위해서 일반적으론 새로운 클래스를 만드는 방식을 사용한다. 이어 새로운 클래스를 사용하는 방법으론 대표적으로 상속이 있다.
- 객체지향에서는 상속외에도 코드를 효과적으로 재사용할 수 있는 방법이 한가지 더있는데 새로운 클래스의 인스턴스 안에 기존 클래스의 인스턴스를 포함시키는 방법으로 흔히 '합성'이라고 부른다
- 여기선 상속과 합성의 장단점을 비교할것이다.
- 코드를 재사용하려는 강력한 동기 이면에는 중복된 코드를 제거하려는 욕망이 숨어 있다. 우선 중복된 코드의 문제점 부터 본다.
 ## 10-1 상속과 중복 코드
   ### DRY 원칙
   > 중복코드의 문제점은 여러가지가 있지만 이것을 제거해야하는 가장 큰 이유로는 중복코드는 변경을 방해한다는 것이다.
   - 정확히는 구현된코드는 언젠가 변경될것이라고 생각하는것이 현명하다
   - 그런 이유에서 중복 코드가 가지는 가장 큰문제를 더 자세하게 설명하면 코드를 수정하는데 필요한 노력을 몇 배로 증가 시키고 읽기 힘들고 꼬여있는 코드를 만들 가능성을 높힌다
   - 신뢰할 수 있고 수정하기 쉬운 소프트웨어를 만드는 효과적인 방법중 하나는 중복을 제거하는 것이다.
   - 이것을 위해서 DRY 원칙을 지켜야 한다고 말한다 DRY 원칙의 핵심은 '코드 안에 중복이 존재해서는 안된다는 것이다.'
   ### 중복과 변경
   > 중복코드의 문제점을 보기위해 한달에 한번씩 가입자별로 전화요금을 계산하는 간단한 어플리케이션이다. 10초당 5원의 통화료가 부과되는 서비스다.예제를 보면 알겠지만 Call, Phone, NightlyDiscountPhone이 있다
   - 우선 처음엔 Call과 Phone 만 있어서 두개만으로 코드 구현이 가능했다. 그런데 '심야 할인 요금제'라는 새로운 방식을 추가해야 하는 상황이 생겨 가장 간단하게 Phone을 복사해 NightlyDiscountPhone라는 새로운 클래스를 만들었다
   - NightlyDiscountPhone는 Phone을 복사(오마쥬)했기 때문에 굉장히 유사하다.
   - 그런데 이번엔 통화요금에 세금이 부과되어 그것을 포함해서 계산해야 한다. 부과되는 세율은 가입자의 폰마다 달라야 하기 때문에 taxRate라는 변수를 예제에 추가한다
   - 예제의 수정전 커밋하고 수정진행했다. 결국 수정된 내용을 보면 NightlyDiscountPhone와 Phone 둘다 변경되야 한다.
   - 중복 코드가 늘어날 수록 어플리케이션은 변경에 취약해지고 버그가 발생할 확률이 높아진다는 것이다. 또한 버그수는 늘어나고 코드변경 시간은 느려질 것이다.
   - 두 클래스 사이의 중복 코드를 제거하는 한가지 방법은 클래스를 하나로 합치고 타입코드(enum사용)를 사용하는 것이지만 이것은 낮은 응집도와 높은 결합도라는 문제에 시달리게 된다.
   - 상속을 이용하게 되면 NightlyDiscountPhone이 Phone을 상속하는 구조가 될것이다. 다만 앞에서도 보았지만 상속을 염두하고 설계되지 않은 클래스를 상속으로 재사용하는 것은 실패할 확률이 크고 무엇보다 결합도가 높아지게 된다.
 ## 10-2 취약한 기반 클래스 문제
   - 앞에서의 상속에 대한 문제점을 예시와 함께 상세하게 보여준다.
   - 불필요한 인터페이스를 상속했을 때 자바 초기버전의 vector와 stack의 예시를 들어 설명한다. 간단하겐 vector의 기존 인터페이스로 인해 stack이 사용하지 않는 메서드들도 상속되어 stack의 정책이 무너지는 경우가 발생한다.
   - 메서드 오버라이딩 오작동에 대한 문제도 있는데 부모클래스 인터페이스의 구조에 의해 오버라이딩한 메서드의 동작이 예상과는 전혀 다른 결과가 발생할 수 있다는 예시였다.
   - 부모클래스와 자식클래스의 동시 수정 문제인데 음악 목록을 추가할 수 있는 플레이리스트 구현하는 간단한 예제로 예시를 들었다.
   - 위 내용을 정리하면 부모클래스에 대한 정책변경으로 구현이 변경된다면 자식 클래스는 메서드를 오버라이딩 하거나 불필요한 인터페이스를 상속 받지 않았음에도 부모 클래스의 수정으로 자식클래스 역시 수정되어야 할 수도 있다는 사실을 보여준다리
 ## 10-3 Phone 로직 예제
   > 10-1,2 는 상속으로 인해 발생하는 취약한 기반 클래스 문제의 다양한 예였다. 이제 다시 Phonerhk NightlyDiscountPhone의 내용으로 돌아와 상속으로 인한 피해를 최소화 할수 있는 방법을 살펴본다
   ### 추상화에 의존하자
     - NightlyDiscountPhone의 가장 큰 문제점은 Phone에 강하게 결합돼 있기 때문에 phone이 변경될셩우 함께 변경될 가능성이 높다는 것이다.
     - 이러한 문제(높은 결합도)를 해결하는 가장 일반적인 방법은 자식 클래스가 부모클래스의 구현이 아닌 추상화에 의존하도록 만드는 것이다.
     - 코드 중복을 제거하기 위해 상속을 도입할 대 따르는 두가지 원칙이 있다
       - 두 메서드가 유사하게 보인다면 차이점을 메서드로 추출하라. 메서드 추출을 통해 두 메서드를 동일한 형태로 보이도록 만들수 있다
       - 부모 클래스의 코드를 하위로 내리지 말고 자식 클래스의 코드를 상위로 올려라. 이 방법이 재사용성과 응집도 측면에서 더 뛰어난 결과를 얻을 수 있다.
   ### 차이를 메서드로 추출하라
     - 가장 먼저 할 일은 중복 코드 안에서 차이점을 별도의 메서드로 추출하는 것이다. 이건 '변하는 부분을 찾고 이를 캡슐화 하라' or '변하는 것으로부터 변하지 않는 것을 분리하라'라는 내용을 메서드 수준에서 적용한 것이다.
     - Phone과 NightlyDiscountPhone의 코드로 부터 본다. 예제 step1
     - 우선 두 클래스의 메서드에서 다른 부분을 별도의 메서드로 추출하는 것이다. 이경우 calculateFee의 for문 안에 구현된 요금 계산 로직이 서로 다르단 걸 알 수 있다. 이름은 calculateCallFee 이다.
     - step2의 Phone과 NightlyDiscountPhone 코드를 본다면 새로운 메서드가 생겼을 것이다. 이로서 두 클래스의 다른 부분을 격리 시켜놓았고 같은 코드를 부모클래스로 올리는 일이 남았다
   ### 중복 코드를 부모 클래스로 올려라
     - 추상클래스로 구현하는 것이 옳다. Phone과 NightlyDiscountPhone이 AbstractPhone이라는 추상 클래스를 상속받도록 수정한다.
     1) AbstractPhone에 공통된 메서드인 calculateFee를 옮긴다. 인스턴스 변수 보다 메서드를 먼저 옮기는 것이 필요한 인스턴스 변수를 컴파일 에러로 판단하기 쉽기 때문이다. 추가로 꼭 필요한 변수만 이동시킬수 있는 점도 있다.
     2) Phone과 NightlyDiscountPhone에 calculateFee를 지운다.
     3) AbstractPhone의 calculateFee에 calls를 추가한다.
     4) 이제 남은건 calculateCallFee 메서드가 없다는 에러인데 calculateCallFee는 시그니처는 동일하지만 내부 구현이 다르기 때문에 추상메서드로 선언하고 자식클래스에서 오버라이딩 할 수 있다록 protected로 선언한다.
     - 이제 Phone에는 일반 요금제를 처리하는데 필요한 로직만, NightlyDiscountPhone에는 심야할인 요금제와 관련된 로직만이 존재하게 된다
   - '위로 올리기' 전략은 실패했더라도 수정하기 쉬운 문제를 발생 시킨다. 추상화 할때 빼먹은게 있더라도 하위 클래스가 해당 행동을 필여로 할때가 오면 이문제는 바로 눈에 띈다.
   - 이제 AbstractPhone은 전체 통화 목록을 계산하는 방법이 바뀔경우, Phone은 일반 요금제의 통화 한건을 계산하는 방식이 바뀔경우 NightlyDiscountPhone은 심야할인 요금제의 통화 한건을 계산하는 방식이 바뀔경우에만 변경되게 된다
   - 세클래스 각각 하나의 변경 이유만을 가진다. 이 클래스 들은 단일 책임 원칙을 준수하기 때문에 응집도가 높다.
   ### 세금 추가하가
     - 정말 잘 변경 될까? 실제로 해보기 전까진 모른다. 다만 이름들이 아쉽기 때문에 step3에선 클래스 네임을 조금 바꾸었다.
     - 추상클래스인 Phone을  수정하면 모든 자식클래스 간에 수정사항을 공유할 수 있을 것이다. 요금에 세금이 부과되도록 인스턴스 변수 taxRate와 calculateFee메서드를 수정한다.
     - 추가하면 끝날까? 아니다. 부모클래스의 taxRate 변수가 추가 됐고 이에 대한 생성자가 생겨났으니 자식 클래스들은 taxRate에 대한 초기화역시 진행해야 한다.
     - 클래스는 메서드만 아니라 인스턴스 변수도 함께 포함한다. 인스턴스 변수 목록이 변하지 않는 상황에서 객체의 행동만 변경된다면 상속 계층에 속한 각 클래스를 독립적으로 진화시킬수 있다.
     - 다만 인스턴스 변수가 추가되는 경우는 다르다. 책임을 아무리 잘 분리하더라도 인스턴스 변수 추가는 종종 상속계층 전반에 변경을 유발할 수있다.
     - 하지만 인스턴스 초기화 로직을 변경하는게 동일한 세금계산 로직을 중복시키는 것 보다는 훨씬 현명한 선택이다.

# 11장 합성과 유연한 설계
> 상속에서 부모 클래스와 자식 클래스 사이의 의존성은 컴파일타입에 해결되지만 합성에서 두 객체 사이이ㅢ 의존성은 런타임에 해결된다 상속관계는 'is-a관계' 라고 부르고 'has-a관계'라고 부른다.
- 합성을 이용하면 포함된 객체의 내부 구현이 변경되더라도 영향을 최소화 할 수 있기 떄문에 변경에 더 안정적인 코드를 얻을 수 있게 된다.
- 상속관계는 클래스 사이의 정적인 관계인데 비해 합성관계는 객체 사이의 동적인 관계다. 즉 코드 작성시점에 결정한 상속 관계는 변경 불가능 하지만 합성관계는 실행시점에 동적으로 변경할 수 있기 떄문이다.
- 설계는 변경과 관련된것이고 변경에 유연하게 대처하라 수 있는 설계가 대부분의 경우에 정답일 가능성이 높다.
- 합성은 포함되는 객체의 퍼블릭 인터페이스를 재사용한다. 따라서 상속대신 합성을 이용하면 구현에 대한 의존서을 인터페이스에 대한 의존성으로 변경할 수 있다.
  ## 11-1 상속을 합성으로
  - 10장에서본 코드 재사용을 위해 상속을 남용했을때의 문제점은 크게 3가지 이다.
    - 불필요한 인터페이스 상속문제 : 자식에게 부적합한 오퍼레이션이 상속되기 떄문에 자식 클래스 인스턴스의 상태가 불안정해지는 문제. vector와 stack의 예시로 살펴봄
    - 메서드 오버라이딩의 오작용 문제: 자식 클래스가 부모 클래스의 메서드를 오버라이딩할 때 자식 클래스가 부모 클래스의 메서드 호출 방법에 영향을 받는 문제
    - 부모 클래스와 자식 클래스의 동시 수정 문제: 부모 클래스와 자식 클래스의 개념적인 결합으로 인해 부모 클래스를 변경할 때 자식 클래스도 함께 변경해야 하는 문제
  - 각 페이지 별로 예제들을 확인하면서 보면 이해가 빠르다. 다만 모든 문제를 해결할수는 없어도 상속보다는 합성을 사용하는것이 더 좋다
  - 향후 변경사항이 있어도 파급효과를 최대한 자식 클래스 내부로 캡슐화 할 수 있기 때문이다. 또한 대부분의 경우 구현(구체 클래스)에 대한 결합 보단 인터페이스에 대한 결합이 더 좋다는 사실도 알아야한다.
  ## 11-2 상속으로 인한 조합의 폭발적인 증가
  - 내용이 방대하고 설명해야 할것이 많아 '정리'의 개념에 맞지 않아 353p ~ 368p까지 내용을 확인해보는게 빠르다
  - 다만 11-2장에서 전체적으로 이야기 하는것은 상속은 결합도를 확실하게 높이고 그안에서 결합도를 낮추려고 해도 지속적인 변경이 생기면 추가해야할 요소는 +가 아니라 x가 되버리는 경우를 예시로 들며 이런경우 상속을 포기하는 편이 현명하다는 이야기를 서술한다
  - 이런경우를 '클래스 폭발'이라고 한다고도 한다.
  ## 11-3 합성 관계로 변경
  > 개인적으로 중간부터 약간 애매한 내용들이 있어 정리하기 적당하지 않은것 같다. 2회독 할때 다시 보고 적절한 이해도가 된다면 정리하려고 한다.
  - 하지만 전체적으로 Phone에 대한 전체 구조를 합성방식으로 변경하는 내용이라 유익하다.
  ## 11-4 믹스인
  > 이부분도 조금 애매하다... 여기는 스칼라라는 언어를 사용해서도 그렇고 개념도 사실 처음보는 내용이라 아리까리하다;; 이해가 어느정도 되면 정리 할것.

# 12장 다형성
> 상속을 사용한다면 사용 목적이 코드를 재사용하기 위해서인지 클라이언트 관점에서 인스턴스들을 동일하게 행동하는 그룹으로 묶기 위해서인지 잘 판단하고 후자에 대해서 사용하는 것이 best practice라고 생각한다
- 이번장은 상속의 관점에서 다형성이 구현되는 기술적 메커니즘을 공부한다
- 이번장은 상속의 일차적인 목적이 코드 재사용이 아닌 서브타입의 구현이라는 사실을 이해하는 것만으로 충분하다.
 ## 12-1 다형성
 - 객체지향에서의 다형성은 '유니버설 다형성', '임시 다형성'으로 나눌수 있다
 - 유니버설 다형성은 다시 '매개변수 다형성'과 '포함 다형성'으로 분류 할 수 있고 임시 다형성은 '오버로딩 다형성'과 '강제 다형성'으로 분류 할 수 있다.
   - 오버로딩 다형성: 하나의 클래스 안에 동일한 이름의 메서드가 존재하는 경우이다.
   - 강제 다형성: 언어가 지원하는 자동적인 타입 변환이나 사용자가 직접 구현한 타입 변환을 이용해 동일한 연산자를 다양한 타입에 사용할 수 있는 방식을 말한다
   - 매개변수 다형성: 제네릭 프로그래밍과 관련이 높은데 클래스의 인스턴스 변수나 메서드의 매개변수타입을 임의의 타입으로 선언후 사용하는 시점에 구체적인 타입으로 지정하는 방식이다.
   - 포함 다형성: 메세지가 동일하더라도 수신한 객체의 타입에 따라 실제로 수행되는 행동이 달라지는 능력을 의미한다. 일반적으로 지금까지 공부했던 다형성이다.
 ## 12-2 상속의 양면성
 - 상속을 이용하면 부모 클래스에서 정의한 모든 데이터를 자식 클래스의 '인스턴스'에 자동으로 포함시킬 수 있다. 이것이 데이터 관점의 상속이다.
 - 데이터 뿐만 아니라 정의한 일부 메서드 역시 자동으로 자식 클래스에 포함 시킬수 있다. 이것이 행동 관점의 상속이다.
 - 상속의 목적은 코드 재사용이 아닌 프로그램을 구성하는 개념들을 기반으로 다형성을 가능하게 하는 타입계층을 구축하기 위한 것이다.
 - 이 장은 상속의 메커니즘을 이해하는 데 필요한 몇가지 개념을 살펴본다.
 - 이 부분은 393 ~ 403p까지의 그림과 표식들 보기 그림과 함께봐야 재대로 정리되는 챕터
 ## 12-3 업캐스팅과 동적 바인딩
  ### 같은 메시지 다른 메서드
  ```
  Professor professor = new Professor("daicstra", new GradeLecture("알고리즘", 70, Arrays.asList(new Grade("A", 100, 95),new Grade("B", 94, 80)), Arrays.asList(81,95,76)))
  ```
  - 위 예제를 보면(많이 더럽지만...) Professor의 생성자는 Lecture 타입의 파라미터를 받지만 실제론 GradeLecture를 인자로 할당해 주고 있다
  - 이렇게 코드 안에서 선언된 참조 타입과 무관하게 실제로 메시지를 수신하는 객체의 '타입'에 따라 실해되는 메서드가 달라질수 있는 것은 업캐스팅과 동적 바인딩이라는 메커니즘 때문이다.
  - 동일한 수신자에게 동인한 메시지를 전송하는 동일한 코드를 이용해 서로 다른 메서드르 실행할 수 있는 이유는 업캐스팅과 동적 메서드 탐색이라는 기반 메터니즘이 존재하기 때문이다.
  - 업캐스팅은 서로다른 클래스의 인스턴스를 동일한 타입에 할당하는 것을 가능하게 해준다. 즉 부모 클래스 타입으로 선언된 변수에 자식 클래스의 인스턴스를 할당하는 것이 가능하다
  - 동적 메서드 탐색은 부모 클래스의 타입에 대해 메시지를 전송하더라도 실행 시에는 실제 클래스를 기반으로 실행될 메서드가 선택되게 해준다.
 - **해당 내용은 개념적인 내용을 파고들어가는 부분이라 정리된 걸 보려고 하기 보단 책을 직접 봐야한다. 예시와 함께 주석을 봐야 이해도가 재대로 될것 같다.**
